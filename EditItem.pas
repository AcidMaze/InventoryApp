...
begin
  ...
  begin
    ShowMessage('Ошибка! Введите новое название предмета.');
    exit;
  end;
  if (Edit2.Text = '0') or (Length(Edit2.Text) = 0) or (Pos(' ',Edit2.Text) > 0) or (Pos('-',Edit2.Text) > 0) then
  begin
    ShowMessage('Ошибка! Количество не может равняться нулю или быть отрицательным.');
    exit;
  end;
  if (Length(Edit3.Text) = 0) or (Pos(' ',Edit3.Text) > 0) or (Pos('-',Edit3.Text) > 0) then
  begin
    ShowMessage('Ошибка! Введите корректную цену.');
    exit;
  end;
  if (Length(Edit4.Text) = 0) then
  begin
    ShowMessage('Ошибка! Введите нового производителя.');
    exit;
  end;
  isImage := false;
  ...
  ShowMessage('Все изменения успешно применены!');
  Edit1.Clear;
  Edit2.Clear;
  Edit3.Clear;
  Edit4.Clear;
...
begin
  ...
  begin
    ...
    DataModule2.DS_ItemsList.DataSet.Refresh;
    ShowMessage('Изображения предмета успешно измененено.');
  end;
end;

procedure TForm7.ComboBox1Change(Sender: TObject);
begin
  if Edit4.Text = '1' then Edit4.Text := 'Мебель'
  else if Edit4.Text = '2' then Edit4.Text := 'Канцтовары'
  else if Edit4.Text = '3' then Edit4.Text := 'Бытовая химия'
  else if Edit4.Text = '4' then Edit4.Text := 'Компьютерные комплектующие'
  else if Edit4.Text = '5' then Edit4.Text := 'Комьютерная переферия'
  else if Edit4.Text = '6' then Edit4.Text := 'Ноутбуки'
  else if Edit4.Text = '7' then Edit4.Text := 'Системные блоки';
end;
...
begin
  ...
  begin
    ...
    ShowMessage('Все изменения были отменены!');
  end;
  Form2.Enabled := true;
end;
